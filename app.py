...
def screen_script_converter():
    ...
    Ported from the standalone Orthographizer app. Deliberately kept as one
    self-contained wizard-style screen (not the config/output two-column
    layout used by database-driven tasks) since it doesn't operate on
    st.session_state.database at all -- it has its own upload flow that
    accepts more flexible input than Excelerant's shared lexical database
    format.
    """
