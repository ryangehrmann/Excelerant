...
Read .txt and .docx text files, split into (token, is_source_script) pairs.

Source-script detection is parameterized on the selected pipeline's
...
def _contains_script(text: str, ranges: list[tuple[int, int]]) -> bool:
    return any(any(lo <= ord(c) <= hi for lo, hi in ranges) for c in text)


def _split_to_tokens(text: str, script_ranges: list[tuple[int, int]]) -> list[tuple[str, bool]]:
    """
    Split text on whitespace/line breaks into tokens.
    Returns list of (token, is_source_script) tuples, preserving whitespace
    tokens so the caller can reconstruct line structure.
    """
    ...
    for part in parts:
        if not part:
            continue
        tokens.append((part, _contains_script(part, script_ranges)))
    return tokens


def _paragraphs_to_tokens(
    paragraphs: list[str], script_ranges: list[tuple[int, int]]
    ...
    for para in paragraphs:
        stripped = para.strip()
        if stripped:
            tokens = _split_to_tokens(stripped, script_ranges)
...
def read_txt(
    file_obj, script_ranges: list[tuple[int, int]]
    ...
    Returns:
        (paragraphs, error) — paragraphs is a list-of-lists of (token, is_source_script).
    """
    raw = file_obj.read()
    ...
    return _paragraphs_to_tokens(paragraphs, script_ranges), None
...
def read_text_file(
    file_obj, filename: str, script_ranges: list[tuple[int, int]]
) -> tuple[list[list[tuple[str, bool]]] | None, str | None]:
    """
    ...
    if lower.endswith(".docx"):
        return read_docx_text(file_obj, script_ranges)
    elif lower.endswith(".txt"):
        return read_txt(file_obj, script_ranges)
    else:
