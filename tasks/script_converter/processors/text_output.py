...
Generate output files from a processed list of paragraphs:
  ...
  1. IPA-only (.docx)    — source-script tokens replaced with IPA (and Lao when
                           available); ambiguous wrapped in {}
  2. Interlinear (.html) — each source-script word shown with its IPA gloss below,
...
def _transcribe_token(
    token: str,
    is_source_script: bool,
    pipeline_run_fn,
    flat_dict: dict,
    segments_dict: dict,
) -> tuple[str, bool, str]:
    """
    Transcode a token if it contains source-script text.

    Returns:
        (ipa_text, is_ambiguous, lao_text) — ipa_text is the transcoded IPA
        string (or the original token for non-source-script tokens); is_ambiguous
        is True if any syllable was flagged ambiguous; lao_text is the assembled
        Lao-script form (empty string when the pipeline does not produce Lao output).
    """
...
def build_ipa_only_docx(
    ...
    """
    Build a .docx where each paragraph of source-script text is replaced with IPA.
    ...
    for para_tokens in paragraphs:
        ...
        for token, is_source_script in para_tokens:
            if token.strip() == "":
                # Preserve whitespace as plain text run
                para.add_run(token)
            else:
                ipa, ambiguous, lao = _transcribe_token(
                    token, is_source_script, pipeline_run_fn, flat_dict, segments_dict
                )
                if not is_source_script:
                    para.add_run(token)
                else:
...
_CSS = """
body {
    font-family: 'Segoe UI', 'Noto Sans Khmer', 'Noto Sans Lao', Arial, sans-serif;
    font-size: 19px;
    padding: 2em 3em;
    background: #fff;
    color: #222;
...
def build_interlinear_html(
    ...
    Each source-script word is stacked above its IPA gloss. Non-source-script
    words appear inline with an empty gloss row so vertical alignment stays
    consistent within mixed paragraphs. Purely non-source-script paragraphs
    are plain text. Ambiguous IPA is shown in red and wrapped in {}.
    """
    parts = [
        "<!DOCTYPE html>",
        "<html>",
        "<head>",
        '  <meta charset="UTF-8">',
        "  <title>Interlinear \u2013 Script Conversion</title>",
    ...
    for para_tokens in paragraphs:
        ...
        has_source_script = any(k for _, k in word_tokens)
        ...
        for token, is_source_script in word_tokens:
            ...
            transcribed.append((token, is_source_script, ipa, ambiguous, lao))

        spans = []
        for token, is_source_script, ipa, ambiguous, lao in transcribed:
            orig_esc = _html.escape(token)
            if not is_source_script:
                # Non-source-script token: text always at top, placeholder rows below.
